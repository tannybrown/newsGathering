...
def test_naver_news_api():
    ...
    # POST 요청으로 네이버 뉴스 검색
    print(f"\n1. POST /news/company - {company_name} 뉴스 검색")
    try:
        ...
        if response.status_code == 200:
            ...
            for i, item in enumerate(data['items'][:3], 1):
                ...
                print(f"  원본 링크: {item['originallink']}")
...
def test_naver_news_simple():
    """GET 요청으로 네이버 뉴스 검색 테스트"""
    print(f"\n=== GET 요청 네이버 뉴스 API 테스트 ===")
    
    company_name = "LG전자"
    
    print(f"\n2. GET /news/company/{company_name} - {company_name} 뉴스 검색")
    try:
        response = requests.get(
            f"{BASE_URL}/news/company/{company_name}",
            params={"display": 3, "start": 1}
        )
